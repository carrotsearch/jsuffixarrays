...
import static org.jsuffixarrays.Tools.*;
...
public final class Skew implements ISuffixArrayBuilder
{
    ...
    /**
     * Stably sort indexes from src[0..n-1] to dst[0..n-1] with values in 0..K from v.
     * A constant offset of <code>vi</code> is added to indexes from src.
     */
    private final static void radixPass(
        int [] src, int [] dst, int [] v, int vi, 
        final int n, final int K) 
    {
        ...
        // count occurrences
        for (int i = 0;  i < n;  i++) cnt[v[vi + src[i]]]++;

        // exclusive prefix sums
        for (int i = 0, sum = 0;  i <= K;  i++)
        {
            final int t = cnt[i];  cnt[i] = sum;  sum += t;
        }

        // sort
        for (int i = 0;  i < n;  i++) dst[cnt[v[vi + src[i]]]++] = src[i];
    }
     ...
     */
    static final void suffixArray(int [] s, int [] SA, int n, int K)
    {
        final int n0 = (n+2)/3, n1 = (n+1)/3, n2 = n/3, n02 = n0+n2;

        final int [] s12  = new int[n02 + 3];  s12[n02] =  s12[n02+1] =  s12[n02+2]= 0; 
        final int [] SA12 = new int[n02 + 3]; SA12[n02] = SA12[n02+1] = SA12[n02+2]= 0;
    ...
    public int [] buildSuffixArray(int [] input, int start, int length)
    {
        ...
        suffixArray(input, SA, length, alphabetSize);
