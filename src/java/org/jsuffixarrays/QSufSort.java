 ...
 * Peter Sanders: <tt>http://www.mpi-inf.mpg.de/~sanders/programs/suffix/</tt>
 * <p>
 ...
 * <p>
 * Algorithm modifies input during processing. See
 * {@link QSufSort#preserveInput} and {@link #QSufSort(boolean)}.
 */
public class QSufSort implements ISuffixArrayBuilder {
    ...
    /**
     * If <code>true</code>, {@link #buildSuffixArray(int[], int, int)} uses
     * copy of input so it is left intact.
     */
    private final boolean preserveInput;

    /**
     * Default constructor, sets <code>preserveInput</code> to
     * <code>false</false>.
     */
    public QSufSort() {
    ...
    private int transform(int n, int k, int l, int q) {
        ...
        for (b = d = r = 0; r < n && d <= e && (c = d << s | (k - l)) <= q; ++r) {
            b = b << s | (V[r] - l + 1); /* b is start of x in chunk alphabet.*/
        ...
        if (d <= n) { /* if bucketing possible, compact alphabet.*/
            ...
            for (pi = r, c = b; pi <= n; ++pi) {
                ...
                c = (c & m) << s | (V[pi] - l + 1); /* shift in next old symbol in chunk.*/
            }
            ...
            for (pi = 0, pj = r, c = b; pj <= n; ++pi, ++pj) {
                ...
                c = (c & m) << s | (V[pj] - l + 1); /* shift in next old symbol in chunk.*/
            }
