 ...
 * </tt>
 * <p>
 * This implementation is basically a translation of the C version given by Yuta
 * Mori: <tt>libdivsufsort-2.0.0, http://code.google.com/p/libdivsufsort/</tt>
 * <p>
...
public final class DivSufSort implements ISuffixArrayBuilder {
    ...
    private final static int[] sqq_table = { 0, 16, 22, 27, 32, 35, 39, 42, 45,
    ...
    private final static int[] lg_table = { -1, 0, 1, 1, 2, 2, 2, 2, 3, 3, 3,
     ...
     * <code>ALPHABET_SIZE</code>)</li>
     * <li>length >= 2</li>
     * <li>start == 0 (this could be removed if we alter the original algorithm
     * and add offsets)</li>
     * </ul>
     * <p>
     */
    ...
    public final int[] buildSuffixArray(int[] input, int start, int length) {
        ...
        assertAlways(mm.max < ALPHABET_SIZE, "max alphabet size is "
                + ALPHABET_SIZE);
     ...
     * suffixes.
     */
    private final void constructSuffixArray(int[] bucket_A, int[] bucket_B,
            int n, int m) {
        int i, j, k; // ptr
        int s, c0, c1, c2;
        // (_c1)])
    ...
    */
     ...
     *
     */
    private final void ssSort(final int PA, int first, int last, int buf,
            int bufsize, int depth, int n, boolean lastsuffix) {
        ...
        if (lastsuffix) {
            ...
            for (a = first, i = SA[first - 1]; (a < last)
                    && ((SA[a] < 0) || (0 < ssCompare(p1, p11, PA + SA[a],
                            depth))); ++a) {
                SA[a - 1] = SA[a];
            }
            SA[a - 1] = i;
     ...
     * 
     */
    private final void ssInplaceMerge(int PA, int first, int middle, int last,
            int depth) {
    ...
    */
     ...
     * D&C based merge.
     */
    private final void ssSwapMerge(int PA, int first, int middle, int last,
        ...
        for (check = 0, ssize = 0;;) {
            ...
            if ((last - middle) <= bufsize) {
                ...
                if (((check & 1) != 0)
                        || (((check & 2) != 0) && (ssCompare(PA
                                + getIDX(SA[first - 1]), PA + SA[first], depth) == 0))) {
                    SA[first] = ~SA[first];
                }
                if (((check & 4) != 0)
                        && ((ssCompare(PA + getIDX(SA[last - 1]),
                                PA + SA[last], depth) == 0))) {
                    SA[last] = ~SA[last];
                }
            ...
            if (0 < m) {
                ...
                if (ssCompare(PA + getIDX(SA[middle - 1]), PA + SA[middle],
                        depth) == 0) {
                    SA[middle] = ~SA[middle];
                }
     ...
     */
    private final void ssMergeForward(int PA, int first, int middle, int last,
            int buf, int depth) {
     ...
     */
    private final void ssMergeBackward(int PA, int first, int middle, int last,
            int buf, int depth) {
    ...
    private final void ssInsertionSort(int PA, int first, int last, int depth) {
        ...
        for (i = last - 2; first <= i; --i) {
            for (t = SA[i], j = i + 1; 0 < (r = ssCompare(PA + t, PA + SA[j],
                    depth));) {
                do {
                    SA[j - 1] = SA[j];
                } while ((++j < last) && (SA[j] < 0));
        ...
        for (ssize = 0, limit = ssIlg(last - first);;) {
            ...
            if (limit < 0) {
                ...
                if ((a - first) <= (last - a)) {
                    ...
                    if (1 < (last - a)) {
                        stack[ssize++] = new StackElement(first, a, depth + 1,
                                ssIlg(a - first));
                        first = a;
                        limit = -1;
                    } else {
            ...
            if (a <= d) {
                ...
                a = first + (b - a);
                c = last - (d - c);
                b = (v <= T[Td + SA[PA + SA[a]] - 1]) ? a : ssPartition(PA, a,
                        c, depth);

                if ((a - first) <= (last - c)) {
                    if ((last - c) <= (c - b)) {
                        stack[ssize++] = new StackElement(b, c, depth + 1,
                                ssIlg(c - b));
                        ...
                        stack[ssize++] = new StackElement(c, last, depth, limit);
                        stack[ssize++] = new StackElement(first, a, depth,
                                limit);
                        first = b;
                        last = c;
                        depth += 1;
    ...
    private final int ssPivot(int Td, int PA, int first, int last) {
        ...
        if (t <= 512) {
            if (t <= 32) {
                ...
                t >>= 2;
                return ssMedian5(Td, PA, first, first + t, middle,
                        last - 1 - t, last - 1);
            }
        }
        t >>= 3;
     ...
     */
    private final int ssMedian5(int Td, int PA, int v1, int v2, int v3, int v4,
            int v5) {
        int t;
    ...
    private final int ssPartition(int PA, int first, int last, int depth) {
        ...
        for (a = first - 1, b = last;;) {
            for (; (++a < b)
                    && ((SA[PA + SA[a]] + depth) >= (SA[PA + SA[a] + 1] + 1));) {
                SA[a] = ~SA[a];
            }
            for (; (a < --b)
                    && ((SA[PA + SA[b]] + depth) < (SA[PA + SA[b] + 1] + 1));) {
            }
            if (b <= a) {
                break;
            }
     ...
     *
     */
    private final TRPartitionResult trPartition(int ISAd, int first,
            int middle, int last, int pa, int pb, int v) {
        int a, b, c, d, e, f;// ptr
        int t, s, x = 0;
    ...
    private final void trIntroSort(int ISA, int ISAd, int first, int last,
            TRBudget budget) {
        ...
        for (ssize = 0, limit = trIlg(last - first);;) {
            if (limit < 0) {
                if (limit == -1) {
                    ...
                    TRPartitionResult res = trPartition(ISAd - incr, first,
                            first, last, a, b, last - 1);
                    a = res.a;
                    b = res.b;
                    /* update ranks */
                    if (a < last) {
                    ...
                    if ((a - first) <= (last - b)) {
                        if (1 < (a - first)) {
                            stack[ssize++] = new StackElement(ISAd, b, last,
                                    trIlg(last - b), trlink);
                            last = a;
                            limit = trIlg(a - first);
                        } else if (1 < (last - b)) {
                        ...
                        if (1 < (last - b)) {
                            stack[ssize++] = new StackElement(ISAd, first, a,
                                    trIlg(a - first), trlink);
                            first = b;
                            limit = trIlg(last - b);
                        } else if (1 < (a - first)) {
                            last = a;
                    ...
                    if (first < last) {
                        ...
                        next = (SA[ISA + SA[a]] != SA[ISAd + SA[a]]) ? trIlg(a
                                - first + 1) : -1;
                        if (++a < last) {
                        ...
                        if (budget.check(a - first) != 0) {
                            if ((a - first) <= (last - a)) {
                                stack[ssize++] = new StackElement(ISAd, a,
                                        last, -3, trlink);
                                ISAd += incr;
                                last = a;
                                limit = next;
                                ...
                                if (1 < (last - a)) {
                                    stack[ssize++] = new StackElement(ISAd
                                            + incr, first, a, next, trlink);
                                    first = a;
                                    limit = -3;
                                } else {
                                    ISAd += incr;
            ...
            // partition
            TRPartitionResult res = trPartition(ISAd, first, first + 1, last,
                    a, b, v);
            a = res.a;
            b = res.b;
            ...
            if ((last - first) != (b - a)) {
                ...
                if ((1 < (b - a)) && ((budget.check(b - a) != 0))) {
                    if ((a - first) <= (last - b)) {
                        if ((last - b) <= (b - a)) {
                            if (1 < (a - first)) {
                                stack[ssize++] = new StackElement(ISAd + incr,
                                        a, b, next, trlink);
                                stack[ssize++] = new StackElement(ISAd, b,
                                        last, limit, trlink);
                                last = a;
                            } else if (1 < (last - b)) {
                                stack[ssize++] = new StackElement(ISAd + incr,
                        ...
                        if ((a - first) <= (b - a)) {
                            if (1 < (last - b)) {
                                stack[ssize++] = new StackElement(ISAd + incr,
                                        a, b, next, trlink);
                                stack[ssize++] = new StackElement(ISAd, first,
                                        a, limit, trlink);
                                first = b;
                            } else if (1 < (a - first)) {
                                stack[ssize++] = new StackElement(ISAd + incr,
    ...
    private final int trPivot(int ISAd, int first, int last) {
        ...
        if (t <= 512) {
            if (t <= 32) {
                ...
                t >>= 2;
                return trMedian5(ISAd, first, first + t, middle, last - 1 - t,
                        last - 1);
            }
        }
        t >>= 3;
    ...
    private final void trFixDown(int ISAd, int sa, int i, int size) {
        ...
        for (v = SA[sa + i], c = SA[ISAd + v]; (j = 2 * i + 1) < size; SA[sa
                + i] = SA[sa + k], i = k) {
            d = SA[ISAd + SA[sa + (k = j++)]];
    ...
    /**
     */
    private final void trPartialCopy(int ISA, int first, int a, int b,
            int last, int depth) {
        int c, d, e;// ptr
        int s, v;
     ...
     */
    private final void trCopy(int ISA, int first, int a, int b, int last,
            int depth) {
        int c, d, e;// ptr
        int s, v;
